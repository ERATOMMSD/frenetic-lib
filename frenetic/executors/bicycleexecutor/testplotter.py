...
def plot(road_length=400, number_of_points=40):
    ...
    vehicle = cpb.carla_pid_on_bicycle(xs, ys)
    print("here")
    pl.figure()
    pl.subplot(221)
    pl.plot(xs, ys, label="Lane center", marker=".", markersize=5)
    pl.plot(vehicle.pxs, vehicle.pys, color="C1", label="Vehicle")
    pl.xlabel("$x$")
    pl.ylabel("$y$")
    pl.legend()
    pl.subplot(222)
    pl.plot(vehicle.ts, vehicle.vs)
    pl.xlabel("Time (seconds)")
    pl.ylabel("Vehicle speed (m/s)")
    pl.subplot(223)
    pl.plot(vehicle.ts, vehicle.psis)
    pl.xlabel("Time (seconds)")
    pl.ylabel("Vehicle heading angle")
    pl.subplot(224)
    pl.plot(vehicle.ts, vehicle.accels)
    pl.xlabel("Time (seconds)")
